...
module.exports = cls => class VirtualLoader extends cls {
  ...
  // 检查 meta(来自lockfile文件数据) 和 root.edgesOut(来自package.json文件数据) 中的依赖关系是否对应。将结果放到 this[flagsSuspect] 中
  ...
  // 解析meta(来自lockfile)数据，得到 links 和 nodes(Node对象)
  [resolveNodes] (s, root) {
    const links = new Map()
  ...
  // 解析 nodes，在Node对象中挂载父子关系，从而完善整个 Node 树
  [assignParentage] (nodes) {
    for (const [location, node] of nodes) {
      ...
      for (const p of walkUp(dirname(path))) {
        const ploc = relpath(this.path, p) // 获取相对路径
        // 获取并添加父节点
