...
module.exports = cls => class ActualLoader extends cls {
  ...
  async [_loadActual] (options) {
    ...
    // 加载 package.json 文件，生成 Node 节点，在组装成节点树
    // 此时还没有创建叶子节点，node.children 是空的，所有的依赖信息存储在 node.edgeOut 中
    this[_actualTree] = await this[_loadFSNode]({
      path: this.path,
      real: await realpath(this.path, this[_rpcache], this[_stcache]),
    })
    // actualTree 目前只有Root节点
    // console.log(this[_actualTree])
    ...
    // 加载各类 lockfile 信息，添加到节点的 Node 对象 meta 上。本项目中读取的是 node_modules/.package-lock.json
  ...
  async [_loadActualVirtually] ({ root }) {
    ...
    // 解析 _actualTree.meta 数据，将父子依赖关系加载到树中
  ...
  // 读取 package.json 并创建节点。同时按路径做 Node 缓存 
