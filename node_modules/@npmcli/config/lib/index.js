...
// nopt 用于解析 cli 参数，文档: https://www.npmjs.com/package/nopt
// 解析完得到所有参数配置的对象，并且额外包含 argv.remain(剩余未解析的参数), argv.original(原始参数数组), argn.cooked(加工过参数数组，缩写已被映射)
...
class Config {
  ...
  // 这里初始化所有配置项。其中 data 字段只做了结构初始化，并未赋值
  constructor ({
    types,
    shorthands,
    defaults,
    ...
    this.types = types // npm cli 解析配置，作为 nopt 的 knownOpts
    this.shorthands = shorthands // npm cli 解析配置，作为 nopt 的 shortHands
    this.defaults = defaults
    this.log = log
    this.argv = argv // 命令行的参数
    this.env = env
    this.execPath = execPath // node 执行路径
    this.platform = platform
    this.cwd = cwd
    // this.parsedArgv 包含 nopt 解析后的参数。remain\original\cooked
    ...
    this.data = new Map() // 保存了所有 config 的信息 (Map 类型)
    let parent = null
    // 这里逐级初始化 config 数据结构
    ...
    this.list = [] // 保存了所有 config 的信息 (array 类型)
  ...
  // 获取 config 数据的方法。{ key:'键值', where:'type层级' }
  get (key, where) {
    if (!this.loaded)
      throw new Error('call config.load() before reading values')
...
class ConfigData {
  constructor (parent) {
    // 通过 Object.create 创建可以继承的数据，方便通过原型链查找。这个思路很巧妙，值得学习。
